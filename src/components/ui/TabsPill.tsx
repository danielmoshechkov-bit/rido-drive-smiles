          ...
          {flatChildren.map((child, idx) => {
            if (isTabsTrigger(child)) {
              return React.cloneElement(child as React.ReactElement<{ className?: string }>, {
                key: idx,
                className:
                  "px-5 h-11 flex items-center gap-2 rounded-full text-white text-[15px] font-semibold whitespace-nowrap transition-colors " +
                  "data-[state=active]:bg-white data-[state=active]:text-foreground data-[state=active]:font-bold data-[state=active]:shadow-sm " +
