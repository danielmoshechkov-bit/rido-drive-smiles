...
export function generateRentalContractHtml(fd: ContractFill): string {
  const row = (k: string, v?: string) =>
    `<tr><td style="padding:6px 12px;border:1px solid #ddd;width:180px;background:#f5f5f5"><strong>${k}</strong></td><td style="padding:6px 12px;border:1px solid #ddd">${v || '—'}</td></tr>`;
  return `
<div style="font-family:'Times New Roman',Georgia,serif;max-width:700px;margin:0 auto;padding:30px;font-size:13px;line-height:1.8;color:#1a1a1a">
  ...
  <p style="text-align:center;font-weight:bold">Nr ${fd.contract_number || '—'}</p>
  <p style="text-align:center;margin-bottom:25px">zawarta w dniu <strong>${fd.contract_date || '—'}</strong> pomiędzy:</p>
  <div style="margin-bottom:15px;padding:12px;border-left:3px solid #333">
    <p style="margin:0"><strong>${fd.company_name || '—'}</strong></p>
    <p style="margin:2px 0">NIP: ${fd.company_nip || '—'}</p>
  ...
  <div style="margin-bottom:20px;padding:12px;border-left:3px solid #333">
    <p style="margin:0"><strong>${fd.renter_name || '—'}</strong></p>
    <p style="margin:2px 0">telefon: ${fd.renter_phone || '—'}</p>
  ...
  <p>Od <strong>${fd.period_from || '—'}</strong> do <strong>${fd.period_to || '—'}</strong>.</p>
  <h2 style="text-align:center;font-size:14px">§3 Czynsz i kaucja</h2>
  <p>Stawka: <strong>${fd.rate || '—'}</strong>. Kaucja: <strong>${fd.deposit || '—'} zł</strong>.</p>
