...
export function FullscreenMapView({
  ...
  const showInfoWindow = useCallback(
    (map: google.maps.Map, iw: google.maps.InfoWindow, listing: PropertyListingForMap) => {
      iw.setContent(
        `<div style="max-width:280px;font-family:system-ui,sans-serif;">${listing.photos?.[0] ? `<img src="${listing.photos[0]}" style="width:100%;height:120px;object-fit:cover;border-radius:8px 8px 0 0;" />` : ""}<div style="padding:12px;"><h3 style="margin:0 0 6px;font-size:14px;font-weight:600;">${listing.title}</h3><div style="font-size:18px;font-weight:700;color:#7c3aed;">${formatPriceFull(listing.price)}</div><div style="font-size:12px;color:#6b7280;">${listing.areaM2} mÂ² ${listing.rooms ? `â¢ ${listing.rooms} pok.` : ""} â¢ ${listing.location}</div>${onViewListing ? `<button onclick="window.__ridoViewListing('${listing.id}')" style="margin-top:8px;padding:6px 16px;background:#7c3aed;color:white;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;">Zobacz szczegÃ³Åy</button>` : ""}</div></div>`
      );
    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-background flex flex-col">
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="hidden sm:inline">Dodaj ogÅoszenie</span>
            </Button>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              <X className="h-4 w-4" />
              <span className="hidden sm:inline">Zamknij mapÄ</span>
            </Button>
          </div>
        </div>
      </header>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="text-center mb-1">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-1">
                ZnajdÅº wymarzonÄ <span className="text-primary">nieruchomoÅÄ</span>
              </h1>
              <p className="text-muted-foreground">Mieszkania, domy, dziaÅki i lokale od zweryfikowanych agencji</p>
            </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="secondary" className="rounded-full px-3 py-1 text-xs">
                    {filteredListings.length} ogÅoszeÅ
                  </Badge>
                </div>
              </div>
            </div>
      ...
      {drawingMode && (
        <div className="bg-primary/10 border-b px-4 py-2 text-center text-sm font-medium text-primary">
          ...
          Rysuj obszar na mapie â kliknij i przeciÄgnij
        </div>
      )}

      <section className="container mx-auto px-4 py-3 md:py-4 flex-1 flex flex-col overflow-hidden">
        <div className="mb-2 flex flex-col gap-2 md:flex-row md:items-center md:justify-between shrink-0">
          <div>
            <p className="text-sm text-muted-foreground">Widok mapy</p>
            <h2 className="text-2xl font-bold text-foreground">{filteredListings.length} ogÅoszeÅ</h2>
          </div>
              ...
              <div className="p-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {selectedListing.areaM2}mÂ² {selectedListing.rooms ? `â¢ ${selectedListing.rooms} pok.` : ""}
                  </span>
                </div>
                {onViewListing && (
                  <Button
          ...
          <div className="flex-1 overflow-y-auto">
            {paginatedSideListings.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">Brak ogÅoszeÅ do wyÅwietlenia</p>
              </div>
            ) : (
