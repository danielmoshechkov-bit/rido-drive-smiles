...
export function RidoAIChatPanel({ open, onClose }: RidoAIChatPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      {/* Panel — right side, portal-style */}
      <div className="relative ml-auto w-full max-w-lg h-full flex flex-col bg-background border-l shadow-2xl animate-in slide-in-from-right duration-300">

        {/* Header */}
        <div className="flex items-center gap-3 px-4 py-3 border-b bg-background">
          <img src={AVATAR} alt="RidoAI" className="w-8 h-8 rounded-full ring-2 ring-primary/20" />
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={cn('p-2 rounded-lg hover:bg-muted transition-colors', showHistory && 'bg-muted')}
              title="Historia"
            >
        ...
        {/* History overlay */}
        {showHistory && (
          <div className="absolute top-[105px] left-0 right-0 bottom-0 z-10 bg-background flex flex-col">
              ...
              <div className="px-3 py-2 space-y-0.5">
                ...
                {filteredConvs.map(conv => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="truncate">{conv.title || 'Nowa rozmowa'}</p>
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1 hover:bg-destructive/10 rounded flex-shrink-0"
                    >
          ...
          <div className="space-y-4">
            {displayMsgs.map((msg, i) => (
              <div key={i} className={cn('flex gap-2.5', msg.role === 'user' ? 'flex-row-reverse' : 'flex-row')}>
                {msg.role === 'assistant' && (
                  <img src={AVATAR} alt="AI" className="w-6 h-6 rounded-full flex-shrink-0 mt-1" />
                )}
                <div className={cn(
