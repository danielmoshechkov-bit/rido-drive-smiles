...
serve(async (req) => {
  ...
  async function aktywuj(): Promise<Record<string, unknown>> {
    ...
    /**
     * PAS I SZELKI: BRAMKA TAKŻE TUTAJ.
     *
     * `voice-number-activate` sprawdza pakiet, zanim założy zadanie — ale to
     * TEN kod wydaje pieniądze u operatora. Zadanie mogło powstać, zanim flagę
     * włączono, albo zostać dopisane inną drogą; kupno numeru dla warsztatu
     * bez pakietu jest nieodwracalne, a odmowa nie kosztuje nic.
     */
    const { data: ustBramki } = await admin.from("billing_settings")
      .select("agent_bramka_aktywacji").eq("id", true).maybeSingle();
    if (ustBramki?.agent_bramka_aktywacji === true) {
      const { data: maPakiet, error: bladPakietu } = await admin
        .rpc("ma_pakiet_agenta", { p_provider: providerId });
      if (bladPakietu || maPakiet !== true) {
        console.warn("[voice-numbers-worker]", JSON.stringify({
          event: "aktywacja_bez_pakietu_odrzucona", provider_id: providerId,
          blad: bladPakietu?.message ?? null,
        }));
        return { odrzucone: "brak oplaconego pakietu Agent" };
      }
    }
