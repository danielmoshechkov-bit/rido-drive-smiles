...
export const redactPersonalData = (input: string): string => {
  ...
  t = t.replace(new RegExp(`\\b\\w+(?:ego)\\s+(?:${MIESIACE})\\b`, "gi"), P.date);
...
const PRZEPROSINY = /\bprzepraszam\b|\bnie zdążyłem\b|\bmuszę się streścić\b|\bproblem techniczny\b/i;
