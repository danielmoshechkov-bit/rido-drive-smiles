/**
 * Generator HTML faktury — JEDYNE ŹRÓDŁO, wspólne dla przeglądarki i serwera.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * DLACZEGO TU, A NIE W `src/`
 * ═══════════════════════════════════════════════════════════════════════════
 * Faktura musi powstać w DWÓCH miejscach: w panelu (klient klika „Pobierz"
 * albo „Email") i w funkcji brzegowej po opłaconym zakupie, gdzie żadnej
 * przeglądarki nie ma. Skopiowanie szablonu dałoby dwie wersje, które
 * rozjadą się przy pierwszej zmianie wyglądu — i nikt tego nie zauważy,
 * dopóki klient nie porówna dwóch swoich dokumentów.
 *
 * Ten sam ruch zrobiliśmy wcześniej z `invoiceNumbering.ts` z tego samego
 * powodu: dwie implementacje numeracji dałyby dwie faktury o tym samym numerze.
 *
 * `src/utils/invoiceHtmlGenerator.ts` re-eksportuje ten plik, więc front
 * importuje jak dotąd i nie wie o zmianie.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * CZEGO TU NIE MA
 * ═══════════════════════════════════════════════════════════════════════════
 * `printHtmlDocument` i `printInvoice` — jedyne dwie rzeczy w tym pliku, które
 * dotykały `window` i `document`. Zostały w `src/utils/`, bo drukowanie jest
 * czynnością przeglądarki i na serwerze nie znaczy nic.
 *
 * Poza tym przeniesienie jest CO DO ZNAKU: treść funkcji nie została tknięta.
