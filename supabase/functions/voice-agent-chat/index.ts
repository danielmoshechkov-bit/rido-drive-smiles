...
serve(async (req) => {
  ...
  try {
    ...
    if (providerId && calendarAccess) {
      ...
      tools.push({
        name: "create_booking",
        description: "Umów wizytę (rezerwacja w kalendarzu). Wywołaj gdy masz: imię, telefon, datę i godzinę. Nazwisko NIE jest wymagane.",
    ...
    if (providerId && ordersAccess) {
      tools.push({
        ...
        input_schema: { type: "object", properties: {
          customer_name: { type: "string", description: "Samo IMIĘ wystarczy — nazwisko nie jest wymagane." }, customer_phone: { type: "string" }, complaint: { type: "string", description: "Lista usterek/zadań do wykonania — KAŻDA w nowej linii od myślnika '- ' (np. '- stuki w zawieszeniu z przodu\\n- sprawdzić łożyska')" },
    ...
    const execute = async (onText: (delta: string) => void) => {
      ...
      for (let round = 0; round <= voiceRouting.maxToolRounds; round++) {
        ...
        const attempted = await executePhase1Fallback(voiceRouting, async (candidate) => {
          ...
          const request = buildPhase1AnthropicRequest(candidate, apiKey, system, convo, tools, voiceRouting.maxOutputTokens, systemTimeContext);
