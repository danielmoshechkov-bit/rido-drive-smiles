  ...
  try {
    ...
    if (snapshotRaw) {
      snapshotBlok = `\n\n=== CO WIESZ O DZIŚ (dane pobrane przy odebraniu połączenia) ===\n${snapshotRaw}\n`
        ...
        + `- GODZINY WYPOWIADASZ Z POLA "wolne_do_wypowiedzenia" — tam stoją gotowe formy, na przykład "dziewiątej", "dwunastej trzydzieści". Pole "wolne" z zapisem cyfrowym jest tylko do dopasowania tego, co powie klient; NIGDY nie czytasz go na głos i NIGDY nie zamieniasz cyfr na słowa samodzielnie. BŁĄD Z PRAWDZIWEJ ROZMOWY: snapshot podał "12:30", agent powiedział „o półtorej" — godzinę, która nie istnieje — i klient dostał z nią potwierdzenie wizyty.\n`
        ...
        + `- DATA SPOZA TEGO BLOKU WYMAGA NARZĘDZIA. Blok obejmuje czternaście dni. Gdy klient prosi o dzień, którego tu NIE MA — nie wyliczaj daty samodzielnie i NIE POTWIERDZAJ wizyty. Najpierw wywołujesz check_availability, dopiero potem podajesz godzinę. BŁĄD Z SYMULACJI 15.08, POWTÓRZONY W DWÓCH JĘZYKACH: klient poprosił o termin „za miesiąc", agent sam wyliczył datę, ani razu nie sprawdził dostępności i potwierdził rezerwację na dzień, o którym nic nie wiedział. Rezerwacja na niesprawdzony dzień to klient stojący przed zamkniętą bramą.\n`
        + `- DWIE GODZINY NA RAZ, NIGDY TRZY. Nie „maksymalnie trzy", nie „dwie albo trzy" — DOKŁADNIE dwie. Pole "wolne_do_wypowiedzenia" jest ZAPASEM do wyboru, nie listą do odczytania. Wybierasz z niego DWIE i tylko te wypowiadasz. Gdy w polu stoi jedna godzina, podajesz jedną. PRAWDZIWA ROZMOWA 15.08: agent przeczytał wszystkie trzy — "w dziewięć, w dwanaście trzydzieści albo w cztery po południu" — klient milczał dziewięć sekund i odpowiedział czymś, czego transkrypcja nie rozpoznała. Trzy opcje w słuchawce to o jedną za dużo, żeby je zapamiętać.\n`
        + `- PRZY DOPYTANIU POWTARZASZ WSZYSTKIE GODZINY, KTÓRE PADŁY. Jeśli klient nie wybrał jednoznacznie, wymieniasz DOKŁADNIE te godziny, które sam przed chwilą podał — komplet, nie wybór z nich. TEN SAM BŁĄD, TA SAMA ROZMOWA: agent podał trzy godziny, przy dopytaniu wymienił dwie skrajne, a klient chciał tę pominiętą w środku. Godzina, której nie powtórzysz, dla klienta przestaje istnieć.\n`
        ...
        + `- NAZWY USŁUG SĄ PO POLSKU, BO TAK MA JE WPISANE WARSZTAT — to jedyne pole, którego nie tłumaczymy za Ciebie. W rozmowie w innym języku NIGDY nie czytaj polskiej nazwy na głos: nazwij usługę w języku rozmowy własnymi słowami, a z cennika weź tylko cenę i czas. Klient mówi „замена масла" — Ty odpowiadasz „замена масла", nie „Wymiana oleju".\n`
        ...
        + `- NIGDY nie odsyłaj do telefonu ani nie zgaduj ceny, daty czy czasu trwania.\n`;
    }
