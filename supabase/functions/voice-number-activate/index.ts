...
serve(async (req) => {
  ...
  /**
   * BRAMKA: NIE MA PAKIETU — NIE MA NUMERU.
   *
   * Dotąd ta funkcja sprawdzała wyłącznie, czy ktoś jest zalogowany i ma
   * warsztat. Każdy właściciel mógł zamówić numer, a my kupowaliśmy go
   * u operatora za swoje pieniądze.
   *
   * Panel od przebudowy nie pokazuje już takiej możliwości bez pakietu, ale
   * interfejs nie jest zabezpieczeniem — funkcję da się zawołać z pominięciem
   * panelu. Bramka stoi więc TUTAJ, a flaga pozwala ją włączyć osobno.
   *
   * `ma_pakiet_agenta` pyta o LINIĘ PLANU, nie o kolumnę `product_line`,
   * której webhook Stripe nie ustawia — inaczej odmówilibyśmy numeru komuś,
   * kto właśnie zapłacił.
   */
  const { data: ustawienia } = await admin.from("billing_settings")
    .select("agent_bramka_aktywacji").eq("id", true).maybeSingle();
  if (ustawienia?.agent_bramka_aktywacji === true) {
    const { data: maPakiet, error: bladPakietu } = await admin
      .rpc("ma_pakiet_agenta", { p_provider: providerId });
    // Przy pieniądzach brak odpowiedzi zamykamy, nie otwieramy: pomyłka „na tak"
    // wydaje NASZE pieniądze na numer u operatora.
    if (bladPakietu || maPakiet !== true) {
      console.info("[voice-number-activate]", JSON.stringify({
        event: "odmowa_bez_pakietu", provider_id: providerId, blad: bladPakietu?.message ?? null,
      }));
      return json({
        error: "BRAK_PAKIETU",
        message: "Numer przydzielamy do opłaconego pakietu Agent. Wybierz pakiet, a numer zamówi się sam.",
      }, 402);
    }
  }
