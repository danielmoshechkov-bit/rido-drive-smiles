...
/**
 * Skrót numeru dzwoniącego — wyłącznie do liczenia, ile razy ktoś dzwonił.
 *
 * Sól z otoczenia, ze stałą wartością zapasową: bez niej ten sam numer dawałby
 * po restarcie inny skrót i limit przestałby działać po cichu. Skrót nie jest
 * tajemnicą — jest zamiennikiem numeru, żeby numeru nie zapisywać.
 */
  ...
  try {
    ...
    const zbuduj = async () => {
      ...
      if (providerId) {
        ...
        /**
         * DEMO — DWA LIMITY, ZANIM COKOLWIEK SIĘ ZACZNIE.
         *
         * Numer demonstracyjny stoi na stronie `/ai-agent` i każdy może pod
         * niego zadzwonić. Rozmowa jest prawdziwa — z prawdziwym SMS-em
         * i prawdziwą rezerwacją, bo to jest dowód na produkt — ale za minuty
         * płacimy my. Stąd sufit dzienny na numer i sufit na jednego
         * dzwoniącego: formularz na stronie nie chroni przed kimś, kto zobaczy
         * numer raz i poda go dalej.
         *
         * Obie liczby siedzą w `billing_settings`, nie w kodzie — podniesienie
         * limitu to UPDATE, nie wdrożenie.
         *
         * Dzwoniącego rozpoznajemy po SKRÓCIE numeru. Do policzenia, ile razy
         * ktoś dzwonił, wystarczy równość dwóch wartości; pełny numer nie jest
         * do niczego potrzebny, a jego brak znaczy, że wyciek tej kolumny
         * nikomu nie zaszkodzi.
         */
        const { data: numerWarsztatu } = await admin.from("voice_numbers")
          .select("id, phone_number, demonstracyjny")
        ...
        if (numerWarsztatu?.demonstracyjny) {
          const { data: ustawienia } = await admin.from("billing_settings")
            .select("demo_limit_dobowy, demo_limit_na_dzwoniacego").eq("id", true).maybeSingle();
          const limitDobowy = Math.max(0, Number(ustawienia?.demo_limit_dobowy ?? 50));
          const limitNaDzwoniacego = Math.max(0, Number(ustawienia?.demo_limit_na_dzwoniacego ?? 2));
          const doba = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();

          const { count: dzisiaj } = await admin.from("voice_calls")
            .select("id", { count: "exact", head: true })
            .eq("provider_id", providerId).eq("z_dema", true).gte("created_at", doba);

          if ((dzisiaj ?? 0) >= limitDobowy) {
            // ALERT: wpis do dziennika zdarzeń numeru. Zostaje ślad z liczbą,
            // więc widać, czy limit dobija się codziennie, czy raz.
            await admin.from("voice_number_events").insert({
              number_id: numerWarsztatu.id, phone_number: numerWarsztatu.phone_number,
              provider_id: providerId, zdarzenie: "demo_limit_dobowy",
              aktor: "voice-agent-init",
              szczegoly: { rozmow_w_dobie: dzisiaj ?? 0, limit: limitDobowy },
            }).then(({ error }) => {
              if (error) console.error("[voice-agent-init] alert limitu nieudany:", error.code, error.message);
            });
            console.error("[voice-agent-init]", JSON.stringify({
              event: "demo_limit_dobowy_osiagniety", rozmow: dzisiaj ?? 0, limit: limitDobowy,
            }));
            return { zajete: true };
          }

          if (callerId && limitNaDzwoniacego > 0) {
            const skrot = await skrotDzwoniacego(callerId);
            const { count: odNiego } = await admin.from("voice_calls")
              .select("id", { count: "exact", head: true })
              .eq("dzwoniacy_skrot", skrot).gte("created_at", doba);
            if ((odNiego ?? 0) >= limitNaDzwoniacego) {
              console.info("[voice-agent-init]", JSON.stringify({
                event: "demo_limit_dzwoniacego", rozmow: odNiego ?? 0, limit: limitNaDzwoniacego,
              }));
              return { zajete: true };
            }
          }
