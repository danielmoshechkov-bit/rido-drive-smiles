...
> Dwadzieścia pięć znanych sposobów, na jakie kontrola w tym repozytorium
...
| **`sprawdz_sql.py` zielono nad SQL-em, który nie ma prawa się wykonać** | kontrola sprawdza SKŁADNIĘ, nie rozstrzyganie nazw. Migracja scalająca pojazdy przeszła na zielono z `min(uuid)` — funkcji, której w Postgresie NIE MA. Padłaby dopiero przy wykonaniu, na produkcji | zielony wynik, a migracji nikt nie uruchomił | **każdą migrację ruszającą dane przepuść przez `BEGIN … ROLLBACK` na żywej bazie** i porównaj liczby przed i po. To jedyne sprawdzenie, które wie, że funkcja istnieje |
...
| **Brak funkcji w `supabase/config.toml`** | brak sekcji `[functions.nazwa]` NIE znaczy „domyślnie jak reszta" — znaczy **`verify_jwt = true`**. Funkcja ściągnięta z produkcji i wdrożona z repo dostaje bramkę, której wcześniej nie miała: wywołujący bez tokenu dostaje 401, zanim jej kod się uruchomi. Dotyczy `referral-validator` i `vehicle-decode`, odzyskanych 15.09.2026 | funkcja działała na produkcji, a po wdrożeniu „tej samej" treści odpowiada 401 | wpis w `config.toml` NAJPIERW, deploy potem. I ustal, kto ją woła i z jakim tokenem — obie te funkcje nie mają dziś wywołującego w tym repozytorium |
